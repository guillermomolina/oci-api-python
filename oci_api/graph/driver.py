...
class Driver(metaclass=Singleton):
    ...
    def load_filesystem(self, filesystem_json, layer=None):
        ...
        filesystem = Filesystem(filesystem_id, layer)
    ...
    def load_layer(self, layer_json, filesystem):
        ...
        layer = Layer(layer_descriptor, filesystem, images)
    ...
    def layer_to_json(self, layer):
        layer_json = {'descriptor': layer.descriptor.to_dict()}
    ...
    def create_filesystem(self, layer=None):
        ...
        filesystem = Filesystem(generate_random_sha256(), layer)
        filesystem.create()
    ...
    def remove_filesystem(self, filesystem):
        ...
        if filesystem_id not in self.filesystems:
            raise FilesystemUnknownException('Filesystem (%s) is not in distribution, can not remove'
                % filesystem_id)
        if self.get_child_layer(filesystem) is not None:
    ...
    def create_layer(self, filesystem):
        ...
        try:
            ...
            del self.filesystems[original_filesystem_id]
        except FilesystemInUseException as e:
            filesystem_id = e.args[0]
            log.debug('Filesystem (%s) was already present' % filesystem_id)
    ...
    def remove_layer(self, layer):
        ...
        if layer_id not in self.layers:
            raise LayerUnknownException('Layer (%s) is not in distribution, can not remove'
                % layer_id)
        if len(layer.images) > 0:
    ...
    def add_image_reference(self, layer, image_id):
        ...
        if layer_id not in self.layers:
            raise LayerUnknownException('Layer (%s) is not in distribution, can not add image ref'
                % layer_id)
